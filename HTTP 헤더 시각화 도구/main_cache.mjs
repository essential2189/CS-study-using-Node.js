...
import { LfuCache } from "./LFU_cache.mjs";
import { test_cache } from "./test_cache.mjs";
...
async function main(data) {
  const myLFU = new LfuCache(MAX_CACHE);
  let count_cache = 0;
  fs.mkdirSync(path.join(__dirname, "/downloads"), { recursive: true });

  //   let data_list = getData(data);
  let data_list = test_cache; // 임의의 중복된 URL
  ...
  for (let i = 0; i < data_list.length; i++) {
    ...
    if (url.includes("http")) {
      ...
      let start = new Date();
      let cache = myLFU.get(url);
      if (cache !== null) {
        let end = new Date();
        printReulst(cache[0], cache[1], cache[2], cache[3], end - start, end - start, cache[6]);
        console.log(">> 캐시됨\n\n");
        count_cache++;
        if (cache[6] >= max_size) {
          max_size = cache[6];
          max_size_url = filename + " (캐시됨)";
        }
        continue;
      } else {
        ...
        await download(url, __dirname + "/downloads/" + filename)
          .then((res) => {
            ...
            myLFU.set(url, [url, filename, domain, ext, wait_time, download_time, res.size]);
          })
  ...
  printFinal(domain_list, data_list, image_count, code_count, size_sum, total_time, max_size, max_size_url, max_wait, max_wait_url, max_download, max_download_url, redirect, count_cache);
}
